...
vi.mock('../Blueprint', () => ({
	default: vi.fn().mockImplementation(() => ({
		...
		getV15Decoded: vi.fn(() => ({ blueprint: { label: 'Test Blueprint', icons: [{ signal: { name: 'transport-belt' } }] } })),
...
describe('Create Component - Imgur URL Integration Tests', () =>
{
	let queryClient;
	...
	const fillBasicForm = async (imageUrl = '') =>
	{
		...
		fireEvent.change(blueprintInput, { target: { value: '0eNqVkMEKgzAMht8l5+qoPaie9jKMjTFwWoUa2dLQapl093YOdmNjMHrJ9-f_k5eQu2WaOmfIPBWQ-wn71Ps8H6hN0vWQe9OlaeHPLq7vLq4d5J4pDRKQ1qEOjMZM' } });
	...
	describe('Valid Imgur URL workflows', () =>
	{
		it('should successfully resolve direct image URL', async () =>
		{
			...
			await waitFor(() =>
			{
				expect(vi.mocked(parseImgurUrl.parseImgurUrl)).toHaveBeenCalledWith('https://i.imgur.com/abc123.jpg');
				expect(vi.mocked(imgurResolver.resolveImgurUrl)).toHaveBeenCalledWith('https://i.imgur.com/abc123.jpg');
			}, { timeout: 10000 });
