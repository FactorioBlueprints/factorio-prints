...
const options: VersionOption[] = (gameVersions as GameVersion[]).map((versionObject) => ({
	value: versionObject.versionNumber,
	label: `${versionObject.version1}.${versionObject.version2}.${versionObject.version3}`,
}));

const optionsByValue: Record<number, VersionOption> = options.reduce(
	(acc, option) => {
