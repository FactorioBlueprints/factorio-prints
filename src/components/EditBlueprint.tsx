...
function checkIfBlueprintBookHasEntriesWithNoName(blueprintBook: BlueprintBook): boolean {
  return some(blueprintBook.blueprints, (eachEntry) => {
    if (eachEntry.blueprint_book)
      return checkIfBlueprintBookHasEntriesWithNoName(eachEntry.blueprint_book);
...
function EditBlueprintWrapper() {
  ...
  const { data: blueprintSummary, isLoading: summaryIsLoading } =
  ...
  const isLoading =
    summaryIsLoading || blueprintIsLoading || rawBlueprintIsLoading || allTagsLoading;
    ...
    (formValues: FormValues) => {
      ...
      try {
        ...
        if (
          ...
          uiState.v15Decoded &&
          "blueprint" in uiState.v15Decoded &&
          isEmpty(uiState.v15Decoded.blueprint?.label)
        ) {
        ...
        if (
          ...
          uiState.v15Decoded &&
          "blueprint" in uiState.v15Decoded &&
          isEmpty(uiState.v15Decoded.blueprint?.icons)
        ) {
        ...
        if (
          ...
          warnings.push(
            "Some blueprints in the book have no name. Consider naming all blueprints.",
          );
        }
      } catch (e) {
        warnings.push(
          "Error validating blueprint: " + (e instanceof Error ? e.message : "Unknown error"),
        );
      }
  ...
  const handleForceSaveBlueprintEdits = useCallback(
  ...
  }, [deleteBlueprintMutation, blueprintId, rawBlueprintData?.author?.userId]);
  ...
  if (!user) {
    return (
      <>
        <h1 className="display-4">{"Edit a Blueprint"}</h1>
        <p className="lead">{"Please log in with Google or GitHub in order to add a Blueprint."}</p>
      </>
    );
  }
  ...
  return (
    <>
      ...
      <Modal show={uiState.deletionModalVisible} onHide={handleHideConfirmDelete}>
        ...
        <Modal.Body>
          <p>
            Deleting: <RichText text={form.state.values.title} inline />
          </p>
        ...
        <Modal.Footer>
          <ButtonToolbar>
            ...
            <Button onClick={handleHideConfirmDelete} disabled={deleteBlueprintMutation.isPending}>
              Cancel
            </Button>
          </ButtonToolbar>
        </Modal.Footer>
      ...
      <Container>
        <Row>
          {form.state.errors?.length > 0 && (
            <Alert variant="danger" className="alert-fixed" dismissible>
          ...
          {updateBlueprintMutation.isError && (
            <Alert
              variant="danger"
            ...
            >
              <h4>{"Error updating blueprint"}</h4>
          ...
          {deleteBlueprintMutation.isError && (
            <Alert
              variant="danger"
              className="alert-fixed"
              dismissible
            ...
            >
              <h4>{"Error deleting blueprint"}</h4>
        ...
        <PageHeader
          title={
            <>
              Editing: <RichText text={form.state.values.title} iconSize="large" inline />
            </>
          }
        />
        <Row>
              ...
              <Col sm={{ span: 10, offset: 2 }}>
                <ButtonToolbar>
                  <Button
                    ...
                    size="lg"
                    disabled={
                      updateBlueprintMutation.isPending || deleteBlueprintMutation.isPending
                    }
                  >
                    <FontAwesomeIcon icon={faSave} size="lg" />
                    {updateBlueprintMutation.isPending ? " Saving..." : " Save"}
