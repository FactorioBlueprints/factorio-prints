...
const Create: React.FC = () => {
  const [user] = useAuthState(getAuth(app)) as [
    User | null | undefined,
    boolean,
    Error | undefined,
  ];
      ...
      if (!newParsedBlueprint || !newV15Decoded) {
        setState((prevState) => ({
          ...
          blueprintValidationError:
            "Invalid blueprint string. Please paste a valid Factorio blueprint.",
        }));
        setBlueprintWrapper(null);
        return;
      }
      ...
      setState((prevState) => ({
        ...
        renderedMarkdown:
          prevState.blueprint.descriptionMarkdown || extractedDescription
            ? DOMPurify.sanitize(
                md.render(prevState.blueprint.descriptionMarkdown || extractedDescription),
              )
            : "",
      }));
    },
  ...
  const validateWarnings = useCallback((): string[] => {
    ...
    if (
      blueprint.isBook() &&
      v15Decoded &&
      checkIfBlueprintBookHasEntriesWithNoName(v15Decoded.blueprint_book)
    ) {
