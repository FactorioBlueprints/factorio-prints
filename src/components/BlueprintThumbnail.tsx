...
const BlueprintThumbnail: React.FC<BlueprintThumbnailProps> = ({
  blueprintSummary,
  prioritizeImage = false,
}) => {
  const [user] = useAuthState(auth);
  const [isCollectionHovering, setIsCollectionHovering] = useState(false);
  const { data: userFavoritesData, isSuccess: userFavoritesIsSuccess } = useUserFavorites(
    user?.uid,
  );
  const { data: userCollectionData, isSuccess: userCollectionIsSuccess } = useUserCollection(
    user?.uid,
  );
  const { data: userBlueprintsData, isSuccess: userBlueprintsIsSuccess } = useUserBlueprints(
    user?.uid,
  );
  ...
  return (
    <Card
      ...
      style={{ width: "11rem", backgroundColor: "#1c1e22" }}
    >
      <Link to="/view/$blueprintId" params={{ blueprintId: key }} from="/" preload={false}>
        <Card.Img
          variant="top"
          src={imageUrl}
          className="blueprint-thumbnail-image"
      ...
      <p className="truncate p-1">
        ...
        >
          <FontAwesomeIcon icon={collectionIcon} className="text-error" />
        </span>
        {"  "}
        <SafeOverlayTrigger placement="bottom" overlay={tooltip}>
          <Link to="/view/$blueprintId" params={{ blueprintId: key }} from="/" preload={false}>
