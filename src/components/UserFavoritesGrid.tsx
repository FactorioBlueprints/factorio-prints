...
const UserFavoritesGrid: React.FC = () => {
  const [currentUser] = useAuthState(getAuth(app));
  const { userId } = useParams({ from: "/favorites/$userId" });
  ...
  const { queriesByKey: blueprintQueriesById, blueprintSummaries } = useEnrichedBlueprintSummaries(
    data,
    isSuccess,
  );
    ...
    return (
      <Container fluid>
        <PageHeader
          title={
            <>
              <FontAwesomeIcon icon={faHeart} className="text-warning" /> by{" "}
        ...
        <div className="p-5 rounded-lg jumbotron">
          ...
          <p className="lead">
            Please log in with Google or GitHub to check moderator permissions for viewing this
            user's favorites.
          </p>
        </div>
      </Container>
  ...
  if (moderatorQuery.isLoading) {
    return <LoadingIndicator isLoading={true} message="Checking permissions..." />;
  }

  if (moderatorQuery.error) {
    return (
      <ErrorDisplay
        error={moderatorQuery.error}
  ...
  if (!isModerator) {
    return (
      <div className="p-5 rounded-lg jumbotron">
        <h1 className="display-4">Access Denied</h1>
        <p className="lead">You must be a moderator to view other users' favorites.</p>
      </div>
    );
  }
  ...
  return (
    <Container fluid>
      ...
      <LoadingIndicator isLoading={isLoading} message="Loading favorites..." />
      ...
      <EmptyResults isEmpty={isEmpty} filteredTags={filteredTags}>
      ...
      <Row className="blueprint-grid-row justify-content-center">
        {sortedBlueprints.map((blueprintSummary: EnrichedBlueprintSummary, index) => (
          <ErrorBoundary
            ...
            fallback={
              <div className="blueprint-thumbnail col-auto" style={{ width: "11rem" }}>
                <div className="text-danger p-3">Failed to load blueprint</div>
              </div>
            }
            showDetails={false}
          >
            <BlueprintThumbnail blueprintSummary={blueprintSummary} prioritizeImage={index === 0} />
          </ErrorBoundary>
        ))}
      </Row>
