  ...
  if (!user) {
    return (
      <div className="p-5 rounded-lg jumbotron">
        ...
        <p className="lead">
          Please log in with Google or GitHub to manage and export your collection.
        </p>
      </div>
    );
  }
  ...
  return (
    <Container fluid>
      <PageHeader
        title={
          <>
            <FontAwesomeIcon icon={faSave} className="text-warning" /> by{" "}
      ...
      {status && (
        <Row className="justify-content-center">
          <Alert variant={status.variant} className="w-100">
            {status.message}
          </Alert>
        </Row>
      )}
      ...
      <LoadingIndicator isLoading={isLoading} message="Loading collection..." />
      ...
      <EmptyResults isEmpty={isEmpty} filteredTags={filteredTags}>
        <p>
          Your collection is empty. Use the <FontAwesomeIcon icon={faPlusSquare} /> icon on
          blueprints to add prints you want in your export book.
        </p>
      </EmptyResults>
