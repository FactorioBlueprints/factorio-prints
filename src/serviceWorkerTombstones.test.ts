...
const readPublicFile = (fileName: string) => readFileSync(resolve(process.cwd(), 'public', fileName), 'utf8');
...
describe('service worker tombstones', () => {
	...
	it.each(serviceWorkerFileNames)(
		...
		async (fileName) => {
			const eventListeners = new Map<string, (event: {waitUntil(promise: Promise<void>): void}) => void>();
			...
			const workerGlobal = {
				addEventListener: vi.fn(
					(eventName: string, listener: (event: {waitUntil(promise: Promise<void>): void}) => void) => {
...
describe.runIf(process.env.VERIFY_SERVICE_WORKER_BUILD === 'true')('production service worker tombstones', () => {
	it('copies exactly the JavaScript tombstones to the build without transformation', () => {
		...
		for (const fileName of serviceWorkerFileNames) {
			expect(readFileSync(resolve(process.cwd(), 'dist', fileName), 'utf8')).toBe(readPublicFile(fileName));
		}
	});
});
