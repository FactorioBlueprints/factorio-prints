...
export function deserializeBlueprintNoThrow(
  data: string,
  context?: { blueprintId?: string },
): RawBlueprintData | null {
  try {
...
function extractBlueprint(blueprint: RawBlueprintData, path?: string): RawBlueprintData {
  if (!path) {
    return blueprint;
  }
  ...
  try {
    ...
    for (const part of parts) {
      ...
      if (blueprintEntry.blueprint) {
        ...
        throw new BlueprintError(
          `Invalid path ${path}: no blueprint at index ${part} at ${traversedPath}`,
        );
      }
    }
...
function parseVersion4(versionNumber: number | string | bigint): string {
