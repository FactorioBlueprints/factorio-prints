...
const expressBeltTypes = ["express-splitter", "express-transport-belt", "express-underground-belt"];

const fastBeltTypes = ["fast-splitter", "fast-transport-belt", "fast-underground-belt"];

const slowBeltTypes = ["splitter", "transport-belt", "underground-belt"];

const allBeltTypes = [...expressBeltTypes, ...fastBeltTypes, ...slowBeltTypes];

const generateEntityHistogram = (parsedBlueprint: any): Array<[string, number]> => {
...
const generateItemHistogram = (parsedBlueprint: any): Array<[string, number]> => {
...
const generateTagSuggestions = (title: string, parsedBlueprint: any, v15Decoded: any): string[] => {
  ...
  const generateTagSuggestionsFromProduction = (
    ...
    if (entityCounts["rocket-silo"] > 0) {
      ...
      if (
        recipeCounts["high-tech-science-pack"] > 0 ||
      ...
      recipeCounts["low-density-structure"] > 0 ||
      recipeCounts["rocket-fuel"] > 0 ||
      recipeCounts["rocket-control-unit"] > 0
    ) {
      tagSuggestions.push("/production/rocket parts/");
    } else if (
      recipeCounts["speed-module-3"] > 0 ||
      ...
      recipeCounts["construction-robot"] ||
      ...
      recipeCounts["advanced-oil-processing"] > 0 ||
      ...
      entityCounts["electric-furnace"] > 0 ||
      entityCounts["steel-furnace"] > 0 ||
      entityCounts["stone-furnace"] > 0
    ) {
  ...
  const generateTagSuggestionsFromEntityHistogram = (
    entityHistogram: Array<[string, number]>,
    entityCounts: Record<string, number>,
  ) => {
    ...
    if (
      ...
      (entityCounts["small-lamp"] > 100 &&
        entityHistogram[1] &&
        entityHistogram[1][0] === "small-lamp")
    ) {
  ...
  const generateTagSuggestionsForMods = (
    allGameEntities: string[],
    entityCounts: Record<string, number>,
    title: string,
  ) => {
    ...
    const bobsMod = some(
      allGameEntities,
      (each) => each.startsWith("electronics-machine") || each.startsWith("bob-"),
    );
    const angelsMod = some(allGameEntities, (each) => each.startsWith("angels-"));
    const warehousingMod =
      entityCounts["storehouse-storage"] > 0 || entityCounts["warehouse-storage"] > 0;
    const lightedElectricPolesMod =
      entityCounts["lighted-small-electric-pole"] > 0 ||
  ...
  const generateAllTagSuggestions = (
    entityHistogram: Array<[string, number]>,
    entityCounts: Record<string, number>,
    recipeHistogram: Array<[string, number]>,
    recipeCounts: Record<string, number>,
    allGameEntities: string[],
  ) => {
  ...
  if (v15Decoded) {
    if (parsedBlueprint.isBook()) {
      const blueprintEntities: any[] = fpFlatMap((bp: any) => bp.blueprint?.entities || [])(
      ...
      const sortedEntityPairs = sortBy(entityPairs, (pair: any) => pair[1]) as Array<
        [string, number]
      >;
      const entityHistogram = reverse(sortedEntityPairs) as Array<[string, number]>;

      const blueprintRecipes: any[] = fpFlatMap((bp: any) => bp.blueprint?.entities || [])(
        v15Decoded.blueprint_book.blueprints,
      );
      ...
      const sortedRecipePairs = sortBy(recipePairs, (pair: any) => pair[1]) as Array<
        [string, number]
      >;
      ...
      const allGameEntities = [...Object.keys(entityCountsObj), ...Object.keys(itemCountsObj)];
  ...
  if (parsedBlueprint && parsedBlueprint.isV15()) {
  ...
  for (const tag of tagSuggestions) {
    if (!tag.startsWith("/") || !tag.endsWith("/")) {
      throw new Error(
        `Tag suggestion format error: "${tag}" must have leading and trailing slashes`,
      );
    }
  }
  return tagSuggestions;
