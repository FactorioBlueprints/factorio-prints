...
export const resolveImgurUrl = async (url: string, options: ImgurResolverOptions = {}): Promise<ResolvedImgurImage> => {
	...
	if (lastError instanceof ImgurResolverError) {
		...
		throw new ImgurResolverError(lastError?.message || 'Failed to resolve Imgur URL after retries', 'API_ERROR');
	}
};
