...
describe('imgurResolver - Core Functionality', () => {
	...
	describe('resolveImgurUrl - Basic Success Cases', () => {
		it('should resolve valid image data', async () => {
			...
			expect(mockFetch).toHaveBeenCalledWith('https://factorio-blueprint-playground.pages.dev/imgur-resolver', {
				method: 'POST',
				headers: {
	...
	describe('resolveImgurUrl - Custom Options', () => {
		it('should use custom API endpoint', async () => {
			...
			expect(mockFetch).toHaveBeenCalledWith('https://custom-api.example.com/resolve', expect.any(Object));
		});
	});
	...
	describe('getErrorMessage', () => {
		it('should return user-friendly messages for known error types', () => {
			expect(getErrorMessage(new ImgurResolverError('Not found', 'NOT_FOUND'))).toBe(
				'Image not found. Please check the URL and try again.',
			);

			expect(getErrorMessage(new ImgurResolverError('Access denied', 'ACCESS_DENIED'))).toBe(
				'This image is private or has been deleted.',
			);

			expect(getErrorMessage(new ImgurResolverError('Timeout', 'TIMEOUT'))).toBe(
				'Request timed out. Please try again.',
			);

			expect(getErrorMessage(new ImgurResolverError('Network error', 'NETWORK_ERROR'))).toBe(
				'Network error. Please check your connection and try again.',
			);

			expect(getErrorMessage(new ImgurResolverError('Invalid response', 'INVALID_RESPONSE'))).toBe(
				'Invalid response from server. Please try again later.',
			);

			expect(getErrorMessage(new ImgurResolverError('Custom error', 'API_ERROR'))).toBe('Custom error');
		});

		it('should handle unknown error types', () => {
			expect(getErrorMessage(new Error('Unknown error'))).toBe('An unexpected error occurred. Please try again.');
