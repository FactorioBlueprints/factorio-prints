...
describe("useRawPaginatedSummaries", () => {
  ...
  it("should populate individual blueprint summaries in cache after fetching", async () => {
    ...
    ).toStrictEqual(examplePage1.data.blueprint1);
    expect(
      queryClient.getQueryData(["blueprintSummaries", "blueprintId", "blueprint2"]),
    ).toStrictEqual(examplePage1.data.blueprint2);
  });
  ...
  it("should handle pagination correctly", async () => {
    ...
    expect(
      queryClient.getQueryData(["blueprintSummaries", "blueprintId", "blueprint3"]),
    ).toStrictEqual(examplePage2.data.blueprint3);
    expect(
      queryClient.getQueryData(["blueprintSummaries", "blueprintId", "blueprint4"]),
    ).toStrictEqual(examplePage2.data.blueprint4);
  });

  it("should use custom parameters correctly", async () => {
    const { result } = renderHook(() => useRawPaginatedSummaries(30, "numberOfFavorites"), {
      wrapper,
    });
