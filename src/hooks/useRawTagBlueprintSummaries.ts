...
export const useRawTagBlueprintSummaries = (tagId: string) => {
  if (tagId && (tagId.startsWith("/") || tagId.endsWith("/"))) {
    throw new Error(
      `useRawTagBlueprintSummaries: tagId "${tagId}" should not start or end with a slash. The normalized tag id should be used for queries.`,
    );
  }
  ...
  return {
    ...
    isLoading:
      tagQuery.isLoading ||
      (tagQuery.isSuccess && blueprintIds.length > 0 && blueprintQueries.some((q) => q.isLoading)),
    isError:
      tagQuery.isError ||
      (tagQuery.isSuccess && blueprintIds.length > 0 && blueprintQueries.some((q) => q.isError)),
    isSuccess:
      tagQuery.isSuccess &&
      (blueprintIds.length === 0 || blueprintQueries.every((q) => q.isSuccess)),
    blueprintIds,
  };
};
