  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const snapshot = await get(
        ref(getFirebaseDatabase(), `/users/${userId}/favorites/${blueprintId}`),
      );
...
const useIsBlueprintFavorite = (
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (!userId || !blueprintId) return false;
      const snapshot = await get(
        ref(getFirebaseDatabase(), `/blueprints/${blueprintId}/favorites/${userId}`),
      );
      return snapshot.exists() && snapshot.val() === true;
    },
